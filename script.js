...
// Make print function globally accessible for the inline onclick attribute
window.printStatement = function() {
    const statementContent = document.querySelector('#supplier-statement-results .printable-document');
    if (statementContent) {
        document.getElementById('print-area').innerHTML = statementContent.outerHTML;
        setTimeout(() => window.print(), 100);
    } else {
        console.error("Could not find content to print.");
        alert("Error: Statement content not found. Please regenerate the statement.");
    }
}

document.addEventListener('DOMContentLoaded', () => {
    // !!! IMPORTANT: PASTE YOUR GOOGLE APPS SCRIPT WEB APP URL HERE
    const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwmm0nBUK6ZnX0HMNtbCIBImw4MyfjfQAl_boZ6O1MRlkjL81czymeBS9jkmDzgefQk/exec';
    ...
    async function loadData() {
        ...
        try {
            ...
            document.getElementById('app-loader').innerHTML = `<p style="color:var(--danger-color);font-weight:bold;">${userMsg}</p>`;
        }
    }
    ...
    function openItemSelectorModal() {
        let currentList;
        if(document.getElementById('view-receive').classList.contains('active')) {
            modalContext = 'receive';
            currentList = state.currentReceiveList;
        } else if (document.getElementById('view-transfer').classList.contains('active')) {
            modalContext = 'transfer';
            currentList = state.currentTransferList;
        } else if (document.getElementById('view-issue').classList.contains('active')) {
            modalContext = 'issue';
    ...
    function showView(viewId) {
        ...
        if (viewId === 'receive') document.getElementById('receive-invoice').value = `GRN-${Date.now()}`;
        if (viewId === 'issue') document.getElementById('issue-ref').value = `ISN-${Date.now()}`;
        if (viewId !== 'receive') { state.currentReceiveList = []; renderReceiveListTable(); }
        if (viewId !== 'transfer') { state.currentTransferList = []; renderTransferListTable(); }
        if (viewId !== 'issue') { state.currentIssueList = []; renderIssueListTable(); }
        if (viewId !== 'suppliers') state.invoiceModalSelections.clear();
    ...
    // PART 3 OF 4: RENDERING FUNCTIONS
    ...
    function renderSupplierStatement(supplierCode, startDateStr, endDateStr) { const resultsContainer = document.getElementById('supplier-statement-results'); const listContainer = document.getElementById('supplier-list-container'); const backButton = document.getElementById('btn-show-all-suppliers'); const supplier = findByKey(state.suppliers, 'supplierCode', supplierCode); if (!supplier) { return; } const financials = calculateSupplierFinancials(); const supplierData = financials[supplierCode]; const sDate = startDateStr ? new Date(startDateStr) : null; const eDate = endDateStr ? new Date(endDateStr) : null; if(eDate) eDate.setHours(23, 59, 59, 999); let openingBalance = 0; if (sDate) { supplierData.events.forEach(event => { if (new Date(event.date) < sDate) { openingBalance += event.debit - event.credit; } }); } const filteredEvents = supplierData.events.filter(event => { const eventDate = new Date(event.date); return (!sDate || eventDate >= sDate) && (!eDate || eventDate <= eDate); }); let balance = openingBalance; let tableBodyHtml = ''; if (sDate) { tableBodyHtml += `<tr style="background-color: var(--bg-color);"><td colspan="3"><strong>Opening Balance as of ${sDate.toLocaleDateString()}</strong></td><td>-</td><td>-</td><td><strong>${openingBalance.toFixed(2)} EGP</strong></td></tr>`; } filteredEvents.forEach(event => { balance += event.debit - event.credit; tableBodyHtml += `<tr><td>${new Date(event.date).toLocaleDateString()}</td><td>${event.type}</td><td>${event.ref}</td><td>${event.debit > 0 ? event.debit.toFixed(2) : '-'}</td><td>${event.credit > 0 ? event.credit.toFixed(2) : '-'}</td><td>${balance.toFixed(2)} EGP</td></tr>`; }); let dateHeader = `from ${new Date(supplierData.events[0]?.date || Date.now()).toLocaleDateString()}`; if (sDate && eDate) { dateHeader = `from ${sDate.toLocaleDateString()} to ${eDate.toLocaleDateString()}`; } else if (sDate) { dateHeader = `from ${sDate.toLocaleDateString()}`; } else if (eDate) { dateHeader = `until ${eDate.toLocaleDateString()}`; } resultsContainer.innerHTML = `<div class="printable-document"><div class="printable-header"><div><h2>Statement: ${supplier.name}</h2><p style="margin:0; color: var(--text-light-color);">For period: ${dateHeader}</p></div><button class="secondary no-print" onclick="printStatement()">Print</button></div><p><strong>Date Generated:</strong> ${new Date().toLocaleString()}</p><table><thead><tr><th>Date</th><th>Type</th><th>Reference</th><th>Debit</th><th>Credit</th><th>Balance</th></tr></thead><tbody>${tableBodyHtml}</tbody><tfoot><tr style="font-weight:bold; background-color: var(--bg-color);"><td colspan="5" style="text-align:right;">Closing Balance:</td><td>${balance.toFixed(2)} EGP</td></tr></tfoot></table></div>`; listContainer.style.display = 'block'; resultsContainer.style.display = 'none'; backButton.style.display = 'inline-flex'; }
    ...
    // PART 4 OF 4: EVENT LISTENERS, HELPERS & INITIALIZATION
    ...
    async function handleTransactionSubmit(payload, buttonEl) {
        ...
        if (result) {
            ...
            refreshViewData(currentView); // Refresh the current view with updated stock
            // Optionally, trigger a full background reload later for ultimate consistency
    ...
    function attachEventListeners() {
        ...
        document.getElementById('table-items').addEventListener('click', e => { if (e.target.classList.contains('btn-edit')) { openEditModal(e.target.dataset.type, e.target.dataset.id); } });
        document.getElementById('table-suppliers').addEventListener('click', e => { if (e.target.classList.contains('btn-edit')) { openEditModal(e.target.dataset.type, e.target.dataset.id); } });
        document.getElementById('table-branches').addEventListener('click', e => { if (e.target.classList.contains('btn-edit')) { openEditModal(e.target.dataset.type, e.target.dataset.id); } });
        document.getElementById('table-sections').addEventListener('click', e => { if (e.target.classList.contains('btn-edit')) { openEditModal(e.target.dataset.type, e.target.dataset.id); } });
    ...
    const exportToExcel = (tableId, filename) => { try { const table = document.getElementById(tableId); const wb = XLSX.utils.table_to_book(table, {sheet: "Sheet1"}); XLSX.writeFile(wb, filename); showToast('Exporting to Excel...', 'success'); } catch (err) { showToast('Excel export failed.', 'error'); Logger.error('Export Error:', err); } };
    ...
    const calculateStockLevels = () => {
        ...
        sortedTransactions.forEach(t => {
            ...
            if (t.type === 'receive') {
                ...
                const totalValue = (current.quantity * current.avgCost) + (t.quantity * t.cost);
                const totalQty = current.quantity + t.quantity;
                stock[branchCode][t.itemCode] = { itemCode: t.itemCode, quantity: totalQty, avgCost: totalQty > 0 ? totalValue / totalQty : 0, itemName: item.name };
            } else if (t.type === 'transfer') {
                const fromBranchCode = t.fromBranchCode; const toBranchCode = t.toBranchCode;
                if (!fromBranchCode || !toBranchCode || !stock.hasOwnProperty(fromBranchCode) || !stock.hasOwnProperty(toBranchCode)) return;
                if (stock[fromBranchCode][t.itemCode]) {
                    const fromStock = stock[fromBranchCode][t.itemCode]; fromStock.quantity -= t.quantity;
                    const toStock = stock[toBranchCode][t.itemCode] || { quantity: 0, avgCost: 0, itemName: item.name };
                    const newTotalValue = (toStock.quantity * toStock.avgCost) + (t.quantity * fromStock.avgCost);
                    const newTotalQty = toStock.quantity + t.quantity;
                    stock[toBranchCode][t.itemCode] = { itemCode: t.itemCode, quantity: newTotalQty, avgCost: newTotalQty > 0 ? newTotalValue / newTotalQty : 0, itemName: item.name };
                }
            } else if (t.type === 'issue') {
                const fromBranchCode = t.fromBranchCode;
                if (!fromBranchCode || !stock.hasOwnProperty(fromBranchCode)) return;
                if (stock[fromBranchCode][t.itemCode]) {
                    stock[fromBranchCode][t.itemCode].quantity -= t.quantity;
                }
            }
        });
        return stock;
    ...
    const populateOptions = (el, data, ph, valueKey, textKey) => { el.innerHTML = `<option value="">${ph}</option>`; (data || []).forEach(item => { el.innerHTML += `<option value="${item[valueKey]}">${item[textKey]} (${item[valueKey]})</option>`; }); }
    ...
    const refreshViewData = (viewId) => {
        ...
        switch(viewId) {
            ...
            case 'items': renderItemsTable(state.items); break;
            case 'suppliers': renderSuppliersTable(state.suppliers); populateOptions(document.getElementById('payment-supplier-select'), state.suppliers, 'Select Supplier', 'supplierCode', 'name'); populateOptions(document.getElementById('supplier-statement-select'), state.suppliers, 'Select for Statement', 'supplierCode', 'name'); renderPaymentList(); document.getElementById('supplier-list-container').style.display = 'block'; document.getElementById('supplier-statement-results').style.display = 'none'; document.getElementById('btn-show-all-suppliers').style.display = 'none'; document.getElementById('statement-start-date').value = ''; document.getElementById('statement-end-date').value = ''; break;
            case 'branches': renderBranchesTable(state.branches); break;
            case 'sections': renderSectionsTable(state.sections); break;
            case 'receive': populateOptions(document.getElementById('receive-supplier'), state.suppliers, 'Select Supplier', 'supplierCode', 'name'); populateOptions(document.getElementById('receive-branch'), state.branches, 'Select Branch', 'branchCode', 'name'); renderReceiveListTable(); break;
            case 'transfer': populateOptions(document.getElementById('transfer-from-branch'), state.branches, 'Select Source', 'branchCode', 'name'); populateOptions(document.getElementById('transfer-to-branch'), state.branches, 'Select Destination', 'branchCode', 'name'); renderTransferListTable(); break;
            case 'issue': populateOptions(document.getElementById('issue-from-branch'), state.branches, 'Select Source', 'branchCode', 'name'); populateOptions(document.getElementById('issue-to-section'), state.sections, 'Select Destination', 'sectionCode', 'name'); renderIssueListTable(); break;
