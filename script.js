// --- GLOBALS ---
const GAS_WEB_APP_URL = "https://script.google.com/macros/s/AKfycbx7-qAdfKkjpijamGuTNo48RIYMmC8MNibN2runAu73hbnkLgDJblVzHW2rOH-PlIqr/exec";
...
async function handleLogin(event) {
    ...
    try {
        ...
        try { await dom.notificationSound.play(); } catch (e) {}
...
function renderBranchRequests(requests) { const tbody = document.getElementById('branch-requests-table'); if(!tbody) return; if (!requests || requests.length === 0) { tbody.innerHTML = '<tr><td colspan="3" class="text-center p-4">No submission history.</td></tr>'; return; } tbody.innerHTML = requests.map(r => { const { badgeClass, statusText } = getStatusSummary(r); return `<tr data-request-id="${r.RequestID}" style="cursor: pointer;"><td>${r.RequestID}</td><td>${new Date(r.SubmissionTimestamp).toLocaleDateString()}</td><td><span class="badge ${badgeClass}">${statusText}</span></td></tr>`; }).join(''); }
function renderOfficerRequests(requests) { const tbody = document.getElementById('officer-requests-table'); if(!tbody) return; if (!requests || requests.length === 0) { tbody.innerHTML = '<tr><td colspan="6" class="text-center p-4">No requests found.</td></tr>'; return; } tbody.innerHTML = requests.map(r => { const { badgeClass, statusText } = getStatusSummary(r); const isResponded = !!r.OfficerResponseTimestamp; return `<tr class="${isResponded ? 'table-light text-muted' : ''}"><td>${r.RequestID}</td><td>${new Date(r.SubmissionTimestamp).toLocaleDateString()}</td><td>${r.Branch}</td><td>${r.SubmittedBy}</td><td><span class="badge ${badgeClass}">${statusText}</span></td><td><button class="btn btn-sm btn-primary review-btn" data-request-id="${r.RequestID}"><i class="fas fa-search"></i> ${isResponded ? 'View' : 'Review'}</button></td></tr>`; }).join(''); }
...
function renderOfficerModal(items) {
    ...
    items.forEach((item, index) => {
        tbody.rows[index].querySelector('.item-status-select').value = item.Status;
    });
}
async function saveItemChanges() { const requestID = $('#request-details-modal').data('requestId'); const updates = Array.from(document.getElementById('modal-items-tbody').querySelectorAll('tr')).map(row => ({ itemID: row.dataset.itemId, newStatus: row.querySelector('.item-status-select').value })); if (updates.length === 0) return; const result = await apiRequest('updateItemStatuses', { updates, requestID }, true); if (result.success) { displayMessage(result.message); await loadAndRenderRequests(); } }
async function finalizeForBranch() { const requestID = $('#request-details-modal').data('requestId'); if (!requestID) return; if (confirm(`This will notify the branch that you have responded to Request ${requestID}. Continue?`)) { const result = await apiRequest('finalizeForBranch', { requestID }, true); if (result.success) { displayMessage(result.message); $('#request-details-modal').modal('hide'); await loadAndRenderRequests(); } } }

// --- BRANCH ACTIONS ---
function addToList() { const code = document.getElementById('code').value.trim(); const unitPrice = document.getElementById('unitPrice').value; if (!code || !unitPrice) { displayMessage("Code and Final Unit Price are required.", true); return; } const alternateSupplierCheck = document.getElementById('alternateSupplierCheck').checked; const alternateSupplierName = document.getElementById('alternateSupplierName').value.trim(); let supplier = alternateSupplierCheck && alternateSupplierName ? alternateSupplierName : document.getElementById('supplierName').value.trim(); const finalUnitPrice = parseFloat(unitPrice); const newItem = { code: code, name: document.getElementById('name').value.trim(), supplier: supplier, units: state.lastUsedUnitCount, discount: state.lastUsedDiscount, vat: state.lastUsedVat, piece: finalUnitPrice.toFixed(3), case: (finalUnitPrice * state.lastUsedUnitCount).toFixed(3), type: document.getElementById('type').value, current: document.getElementById('currentPrice').value || '0' }; state.requestList.push(newItem); displayMessage("Item added to list."); clearRequestForm(); renderRequestListTable(); }
async function confirmReceipt() { const requestID = document.getElementById('confirm-receipt-btn').dataset.requestId; if (!requestID) return; const result = await apiRequest('confirmReceipt', { requestID }, true); if (result.success) { displayMessage(result.message); document.getElementById('confirm-receipt-btn').style.display = 'none'; await loadAndRenderRequests(); } }
...
function displayMessage(message, isError = false, duration = 4000) { const toastContainer = document.getElementById('toast-container'); const toastId = 'toast-' + Date.now(); const toastHTML = `<div id="${toastId}" class="toast" role="alert" aria-live="assertive" aria-atomic="true" data-delay="${duration}" data-autohide="true"><div class="toast-body">${message}</div></div>`; toastContainer.insertAdjacentHTML('beforeend', toastHTML); $('#' + toastId).toast('show'); }
function startNotificationPolling() { if (state.notificationInterval) clearInterval(state.notificationInterval); state.notificationInterval = setInterval(async () => { if (document.hidden || !state.currentUser) return; const result = await apiRequest('checkForNotifications', { lastCheck: state.lastNotificationCheck }); if (result.success) { state.lastNotificationCheck = new Date().toISOString(); if (result.data.toastEvents && result.data.toastEvents.length > 0) { displayMessage(result.data.toastEvents.join('\n')); dom.notificationSound.play().catch(e => {}); } if (result.data.modalEvent) { showFullscreenNotification(result.data.modalEvent); } if (result.data.toastEvents.length > 0 || result.data.modalEvent) { await loadAndRenderRequests(); } } }, 15000); }
...
function cacheAppDOMElements() { if (document.getElementById('branch-user-content')) { /* cache branch elements */ } if (document.getElementById('officer-content')) { /* cache officer elements */ } }
function setupEventListeners() {
    dom.loginForm.addEventListener('submit', handleLogin);
    dom.logoutBtn.addEventListener('click', handleLogout);
    document.body.addEventListener('click', (e) => {
        const target = e.target;
        const navBtn = target.closest('#nav-buttons .btn');
        if (navBtn) { document.querySelectorAll('#nav-buttons .btn').forEach(b => b.classList.remove('active')); navBtn.classList.add('active'); if(navBtn.id === 'nav-main-btn') showMainAppView(); if(navBtn.id === 'nav-reports-btn') showReportsView(); if(navBtn.id === 'nav-admin-btn') showAdminView(); return; }
        const toggleBtn = target.closest('.toggle-history-btn');
        if (toggleBtn) { const container = toggleBtn.closest('.card-header').nextElementSibling; const isVisible = container.style.display !== 'none'; container.style.display = isVisible ? 'none' : 'block'; toggleBtn.innerHTML = isVisible ? '<i class="fas fa-eye"></i> Show' : '<i class="fas fa-eye-slash"></i> Hide'; return; }
        if (target.closest('#add-user-btn')) { openUserModal(); return; }
        if (target.closest('#save-user-btn')) { saveUser(); return; }
        if (target.closest('.edit-user-btn')) { openUserModal(JSON.parse(target.closest('.edit-user-btn').dataset.user)); return; }
        if (target.closest('.delete-user-btn')) { deleteUser(target.closest('.delete-user-btn').dataset.logincode); return; }
        ...
        if (target.closest('#finalize-for-branch-btn')) { finalizeForBranch(); return; }
        if (target.closest('#save-item-changes-btn')) { saveItemChanges(); return; }
        if (target.closest('#confirm-receipt-btn')) { confirmReceipt(); return; }
        ...
        if (target.closest('#export-officer-btn')) { exportModalAsJPG('officer-modal-body-exportable', 'request-details'); return; }
        if (target.closest('#export-branch-btn')) { exportModalAsJPG('branch-modal-body-exportable', 'request-history'); return; }
    });
}
